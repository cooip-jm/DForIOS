...
const ringTextSize = 12 // 圆环中心文字大小
const creditTextSize = 20 // 话费文本大小
const textSize = 12 // 文字模式下文字大小
...
if (widgetParam == "1") {
    ...
    bodyStack.addSpacer(8)
    await textLayout(scoreIcon, score.remainTitle, zjifen, score.unit)
} else {
...
async function getData() {
    ...
    var url = 'https://m.client.10010.com/mobileService/home/queryUserInfoSeven.htm?showType=3&version=iphone_c@8.2000&desmobiel=' + Tel;
    var url1 = 'https://act.10010.com/SigninApp/signin/daySign'
    var url2 = 'https://m.client.10010.com/welfare-mall-front/mobile/show/flDetail/v1/0'
    try {
...
async function getzhenjifen() {
    ...
    try {
        ...
        var data2 = await req2.loadJSON();
        if (data2.code === '0') {
            files.writeString(cachePath, JSON.stringify(data2))
            log("==>目前有可兑换积分" + data2.resdata.score.availablescore)
        }
    } catch (e) {
        log("==>数据请求失败，使用缓存数据/" + e)
    }
    return data2.resdata.score.availablescore
